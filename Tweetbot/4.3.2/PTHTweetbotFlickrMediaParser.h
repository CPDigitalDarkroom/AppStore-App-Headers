/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 2:16:29 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/E50E682B-1B17-4DC6-B9DF-B10638C3B840/Tweetbot.app/Tweetbot
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Tweetbot/PTHTweetbotMediaParser.h>

@interface PTHTweetbotFlickrMediaParser : PTHTweetbotMediaParser
+(int)mediumTypeForEntity:(id)arg1 ;
+(void)loadURLForMedium:(id)arg1 response:(/*^block*/id)arg2 ;
+(void)loadPreviewURLDictionaryForMedium:(id)arg1 response:(/*^block*/id)arg2 ;
+(id)parsableHosts;
+(char)sourceFromEntity:(id)arg1 size:(unsigned)arg2 completion:(/*^block*/id)arg3 ;
+(void)_loadMedium:(id)arg1 response:(/*^block*/id)arg2 ;
+(char)loadMediaURLFromEntity:(id)arg1 completion:(/*^block*/id)arg2 ;
@end

