/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 2:22:22 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/B4BDDC65-4400-4BC5-A974-2245FB5B0846/MessengerCompressed-iphoneos-arm64,iphoneos-armv7.app/MessengerCompressed-iphoneos-arm64,iphoneos-armv7
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <MessengerCompressed-iphoneos-arm64,iphoneos-armv7/FBPaymentsFormTextInputHandler.h>

@class FBPaymentsFormTextFieldView;

@interface FBPaymentsFormCardSecurityCodeInputHandler : FBPaymentsFormTextInputHandler {

	char _cardNumberIsRequired;
	int _association;
	FBPaymentsFormTextFieldView* _cardNumberView;

}

@property (assign,nonatomic) int association;                                           //@synthesize association=_association - In the implementation block
@property (nonatomic,retain) FBPaymentsFormTextFieldView * cardNumberView;              //@synthesize cardNumberView=_cardNumberView - In the implementation block
-(void)setCardNumberView:(FBPaymentsFormTextFieldView *)arg1 ;
-(void)doNotRequireCardNumber;
-(id)textInputStateForText:(id)arg1 truncateExceededCharacters:(char)arg2 ;
-(id)processChangeFromState:(id)arg1 toState:(id)arg2 textInputEvent:(int)arg3 ;
-(id)effectiveCharacterSet;
-(id)errorDescriptionForError:(id)arg1 ;
-(unsigned)expectedLengthOfEffectiveCharactersWithInputedEffectiveText:(id)arg1 ;
-(void)setAssociation:(int)arg1 ;
-(char)_isCardAssociatedWithAmericanExpress;
-(int)association;
-(FBPaymentsFormTextFieldView *)cardNumberView;
-(char)isReady;
-(id)init;
-(int)keyboardType;
@end

