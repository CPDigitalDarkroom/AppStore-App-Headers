/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 2:21:44 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/B4BDDC65-4400-4BC5-A974-2245FB5B0846/MessengerCompressed-iphoneos-arm64,iphoneos-armv7.app/MessengerCompressed-iphoneos-arm64,iphoneos-armv7
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class MNProfileImageModel, MNProfileImageViewController;

@interface MNJoinableGroupActionHeaderSpecifier : NSObject {

	MNProfileImageModel* _profileImageModel;
	MNProfileImageViewController* _profileImageViewController;

}

@property (nonatomic,readonly) MNProfileImageModel * profileImageModel;                                //@synthesize profileImageModel=_profileImageModel - In the implementation block
@property (nonatomic,readonly) MNProfileImageViewController * profileImageViewController;              //@synthesize profileImageViewController=_profileImageViewController - In the implementation block
-(MNProfileImageModel *)profileImageModel;
-(id)initWithProfileImageModel:(id)arg1 profileImageViewController:(id)arg2 ;
-(MNProfileImageViewController *)profileImageViewController;
-(char)isEqual:(id)arg1 ;
-(unsigned)hash;
@end

