/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 2:22:49 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/B4BDDC65-4400-4BC5-A974-2245FB5B0846/MessengerCompressed-iphoneos-arm64,iphoneos-armv7.app/MessengerCompressed-iphoneos-arm64,iphoneos-armv7
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <MessengerCompressed-iphoneos-arm64,iphoneos-armv7/FBIntentTarget.h>

@class NSString;

@interface FBGroupReportedPostsIntentTarget : FBIntentTarget {

	char _showGroupContextHeader;
	NSString* _FBID;
	NSString* _source;

}

@property (nonatomic,copy,readonly) NSString * FBID;                     //@synthesize FBID=_FBID - In the implementation block
@property (nonatomic,copy,readonly) NSString * source;                   //@synthesize source=_source - In the implementation block
@property (nonatomic,readonly) char showGroupContextHeader;              //@synthesize showGroupContextHeader=_showGroupContextHeader - In the implementation block
+(id)groupReportedPostsTargetWithFBID:(id)arg1 source:(id)arg2 showGroupContextHeader:(char)arg3 ;
-(NSString *)FBID;
-(id)fallbackURLs;
-(char)showGroupContextHeader;
-(char)isEqual:(id)arg1 ;
-(unsigned)hash;
-(id)description;
-(NSString *)source;
@end

