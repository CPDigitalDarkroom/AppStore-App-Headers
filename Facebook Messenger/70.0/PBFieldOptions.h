/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 2:23:07 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/B4BDDC65-4400-4BC5-A974-2245FB5B0846/MessengerCompressed-iphoneos-arm64,iphoneos-armv7.app/MessengerCompressed-iphoneos-arm64,iphoneos-armv7
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <MessengerCompressed-iphoneos-arm64,iphoneos-armv7/PBExtendableMessage.h>
#import <MessengerCompressed-iphoneos-arm64,iphoneos-armv7/GeneratedMessageProtocol.h>

@class NSString, NSMutableArray, NSArray;

@interface PBFieldOptions : PBExtendableMessage <GeneratedMessageProtocol> {

	unsigned hasPacked_ : 1;
	unsigned hasLazy_ : 1;
	unsigned hasDeprecated_ : 1;
	unsigned hasWeak_ : 1;
	unsigned hasExperimentalMapKey_ : 1;
	unsigned hasCtype_ : 1;
	unsigned packed_ : 1;
	unsigned lazy_ : 1;
	unsigned deprecated_ : 1;
	unsigned weak_ : 1;
	NSString* experimentalMapKey;
	long ctype;
	NSMutableArray* uninterpretedOptionArray;

}

@property (assign) long ctype; 
@property (assign) char packed; 
@property (assign) char lazy; 
@property (assign) char deprecated; 
@property (retain) NSString * experimentalMapKey; 
@property (assign) char weak; 
@property (retain) NSMutableArray * uninterpretedOptionArray; 
@property (readonly) NSArray * uninterpretedOption; 
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
+(id)parseFromData:(id)arg1 ;
+(id)builderWithPrototype:(id)arg1 ;
+(id)parseFromData:(id)arg1 extensionRegistry:(id)arg2 ;
+(id)parseFromInputStream:(id)arg1 ;
+(id)parseFromInputStream:(id)arg1 extensionRegistry:(id)arg2 ;
+(id)parseFromCodedInputStream:(id)arg1 ;
+(id)parseFromCodedInputStream:(id)arg1 extensionRegistry:(id)arg2 ;
+(id)defaultInstance;
+(void)initialize;
+(id)builder;
-(void)writeToCodedOutputStream:(id)arg1 ;
-(long)serializedSize;
-(void)writeDescriptionTo:(id)arg1 withIndent:(id)arg2 ;
-(id)toBuilder;
-(void)setDeprecated:(char)arg1 ;
-(NSArray *)uninterpretedOption;
-(char)hasDeprecated;
-(char)deprecated;
-(NSMutableArray *)uninterpretedOptionArray;
-(void)setHasDeprecated:(char)arg1 ;
-(id)uninterpretedOptionAtIndex:(unsigned)arg1 ;
-(void)setUninterpretedOptionArray:(NSMutableArray *)arg1 ;
-(void)setCtype:(long)arg1 ;
-(void)setPacked:(char)arg1 ;
-(void)setLazy:(char)arg1 ;
-(void)setExperimentalMapKey:(NSString *)arg1 ;
-(void)setWeak:(char)arg1 ;
-(char)hasCtype;
-(long)ctype;
-(char)hasPacked;
-(char)packed;
-(char)hasLazy;
-(char)lazy;
-(char)hasExperimentalMapKey;
-(NSString *)experimentalMapKey;
-(char)hasWeak;
-(char)weak;
-(void)setHasCtype:(char)arg1 ;
-(void)setHasPacked:(char)arg1 ;
-(void)setHasLazy:(char)arg1 ;
-(void)setHasExperimentalMapKey:(char)arg1 ;
-(void)setHasWeak:(char)arg1 ;
-(id)defaultInstance;
-(id)init;
-(char)isEqual:(id)arg1 ;
-(unsigned)hash;
-(id)builder;
-(char)isInitialized;
@end

