/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 2:21:49 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/B4BDDC65-4400-4BC5-A974-2245FB5B0846/MessengerCompressed-iphoneos-arm64,iphoneos-armv7.app/MessengerCompressed-iphoneos-arm64,iphoneos-armv7
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <MessengerCompressed-iphoneos-arm64,iphoneos-armv7/FBValueObject.h>
#import <libobjc.A.dylib/NSCopying.h>

@class NSString;

@interface MNRideServiceSurgeInfo : FBValueObject <NSCopying> {

	float _surgeValue;
	float _surgeThreshold;
	NSString* _surgeDescription;
	NSString* _surgeId;

}

@property (nonatomic,readonly) float surgeValue;                              //@synthesize surgeValue=_surgeValue - In the implementation block
@property (nonatomic,readonly) float surgeThreshold;                          //@synthesize surgeThreshold=_surgeThreshold - In the implementation block
@property (nonatomic,copy,readonly) NSString * surgeDescription;              //@synthesize surgeDescription=_surgeDescription - In the implementation block
@property (nonatomic,copy,readonly) NSString * surgeId;                       //@synthesize surgeId=_surgeId - In the implementation block
-(id)initWithSurgeValue:(float)arg1 surgeThreshold:(float)arg2 surgeDescription:(id)arg3 surgeId:(id)arg4 ;
-(float)surgeValue;
-(float)surgeThreshold;
-(NSString *)surgeDescription;
-(NSString *)surgeId;
@end

