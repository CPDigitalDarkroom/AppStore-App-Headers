/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 2:22:45 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/B4BDDC65-4400-4BC5-A974-2245FB5B0846/MessengerCompressed-iphoneos-arm64,iphoneos-armv7.app/MessengerCompressed-iphoneos-arm64,iphoneos-armv7
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <MessengerCompressed-iphoneos-arm64,iphoneos-armv7/FBNetworkerRequest.h>

@class NSString, NSDictionary;

@interface SRRestAPINetworkRequest : FBNetworkerRequest {

	NSString* _restMethodName;
	NSString* _httpMethod;
	NSDictionary* _params;

}

@property (nonatomic,copy,readonly) NSString * restMethodName;              //@synthesize restMethodName=_restMethodName - In the implementation block
@property (nonatomic,copy,readonly) NSString * httpMethod;                  //@synthesize httpMethod=_httpMethod - In the implementation block
@property (nonatomic,copy,readonly) NSDictionary * params;                  //@synthesize params=_params - In the implementation block
-(id)networkRequest;
-(NSString *)restMethodName;
-(id)initWithRESTMethodName:(id)arg1 httpMethod:(id)arg2 params:(id)arg3 callbackPerformer:(id)arg4 ;
-(NSDictionary *)params;
-(char)isEqual:(id)arg1 ;
-(unsigned)hash;
-(NSString *)httpMethod;
@end

