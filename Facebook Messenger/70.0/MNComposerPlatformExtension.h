/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 2:21:44 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/B4BDDC65-4400-4BC5-A974-2245FB5B0846/MessengerCompressed-iphoneos-arm64,iphoneos-armv7.app/MessengerCompressed-iphoneos-arm64,iphoneos-armv7
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <MessengerCompressed-iphoneos-arm64,iphoneos-armv7/MNAbstractComposerTrayExtension.h>
#import <MessengerCompressed-iphoneos-arm64,iphoneos-armv7/MNPlatformAppActionHandlerListener.h>

@protocol MNPlatformAppActionFactory;
@class MNPlatformAppItem, MNPlatformAppActionHandler, MNPlatformAppManager, MNPlatformAppsRankingGating, NSString;

@interface MNComposerPlatformExtension : MNAbstractComposerTrayExtension <MNPlatformAppActionHandlerListener> {

	MNPlatformAppItem* _appItem;
	MNPlatformAppActionHandler* _actionHandler;
	id<MNPlatformAppActionFactory> _platformAppActionFactory;
	MNPlatformAppManager* _platformAppManager;
	MNPlatformAppsRankingGating* _platformAppsRankingGating;

}

@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(id)initWithAppItem:(id)arg1 actionHandler:(id)arg2 platformActionFactory:(id)arg3 platformAppManager:(id)arg4 platformAppsRankingGating:(id)arg5 ;
-(id)_composerIcon;
-(void)didSelect:(char)arg1 source:(int)arg2 ;
-(char)shouldPresent:(char)arg1 source:(int)arg2 ;
-(char)isEligible;
-(void)_logPlatformAction:(id)arg1 ;
-(id)_platformAppID;
-(void)actionHandlerDidOpenURLReplyWithAttributionID:(id)arg1 ;
-(id)viewControllerForPresentation;
-(char)appIsInstalled;
-(void)actionHandlerDidDismissWithAttributionID:(id)arg1 ;
-(int)category;
-(id)tabBarItem;
-(char)isThirdParty;
-(id)extensionIdentifier;
@end

