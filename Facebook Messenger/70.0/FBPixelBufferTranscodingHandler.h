/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 2:22:49 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/B4BDDC65-4400-4BC5-A974-2245FB5B0846/MessengerCompressed-iphoneos-arm64,iphoneos-armv7.app/MessengerCompressed-iphoneos-arm64,iphoneos-armv7
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <MessengerCompressed-iphoneos-arm64,iphoneos-armv7/MessengerCompressed-iphoneos-arm64,iphoneos-armv7-Structs.h>
#import <MessengerCompressed-iphoneos-arm64,iphoneos-armv7/FBTranscodingHandler.h>

@protocol OS_dispatch_queue, FBTranscodingHandlerDelegate;
@class AVAssetWriter, AVAssetWriterInput, AVAssetWriterInputPixelBufferAdaptor, AVAssetReader, AVAssetReaderTrackOutput, NSObject, NSString;

@interface FBPixelBufferTranscodingHandler : NSObject <FBTranscodingHandler> {

	AVAssetWriter* _writer;
	AVAssetWriterInput* _writerInput;
	AVAssetWriterInputPixelBufferAdaptor* _pixelBufferAdaptor;
	AVAssetReader* _reader;
	AVAssetReaderTrackOutput* _readerTrackOutput;
	opaqueCMSampleBufferRef _firstSampleBuffer;
	int _videoOrientation;
	NSObject*<OS_dispatch_queue> _queue;
	char _done;
	id<FBTranscodingHandlerDelegate> _delegate;
	SCD_Struct_FB51 _presentationTime;

}

@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
@property (nonatomic,readonly) char done;                                                   //@synthesize done=_done - In the implementation block
@property (nonatomic,readonly) SCD_Struct_FB51 presentationTime;                            //@synthesize presentationTime=_presentationTime - In the implementation block
@property (assign,nonatomic,__weak) id<FBTranscodingHandlerDelegate> delegate;              //@synthesize delegate=_delegate - In the implementation block
-(void)startTranscodingWhenReady;
-(id)initWithAssetWriter:(id)arg1 writerInput:(id)arg2 pixelBufferAdaptor:(id)arg3 assetReader:(id)arg4 readerTrackOutput:(id)arg5 firstSampleBuffer:(opaqueCMSampleBufferRef)arg6 videoOrientation:(int)arg7 queue:(id)arg8 ;
-(void)setDelegate:(id<FBTranscodingHandlerDelegate>)arg1 ;
-(void)dealloc;
-(id<FBTranscodingHandlerDelegate>)delegate;
-(char)done;
-(SCD_Struct_FB51)presentationTime;
@end

