/*
* This header is generated by classdump-dyld 1.0
* on Thursday, May 12, 2016 at 1:01:14 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/E107A69B-5FBA-4958-8D2E-301137467F53/Translate.app/Translate
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Translate/PBGeneratedMessage.h>

@class NSString, PBMutableArray;

@interface EnumOptions : PBGeneratedMessage

@property (assign,nonatomic) char hasProto1Name; 
@property (nonatomic,retain) NSString * proto1Name; 
@property (assign,nonatomic) char hasCcAllowUnsafeEnumToString; 
@property (assign,nonatomic) char ccAllowUnsafeEnumToString; 
@property (assign,nonatomic) char hasAllowAlias; 
@property (assign,nonatomic) char allowAlias; 
@property (assign,nonatomic) char hasDeprecated; 
@property (assign,nonatomic) char deprecated; 
@property (assign,nonatomic) char hasUninterpretedOptionArray; 
@property (nonatomic,retain) PBMutableArray * uninterpretedOptionArray; 
+(id)descriptor;
@end

