/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 9:31:56 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/8A9DE0F0-8C0D-4043-A8B5-545C839396E1/Twitter.app/Twitter
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


#import <Twitter/Twitter-Structs.h>
@class TFNTwitterStatus;

@interface TFNTwitterMediaPreviewInfo : NSObject {

	TFNTwitterStatus* _status;
	unsigned _mediaIndex;

}

@property (nonatomic,retain) TFNTwitterStatus * status;              //@synthesize status=_status - In the implementation block
@property (assign,nonatomic) unsigned mediaIndex;                    //@synthesize mediaIndex=_mediaIndex - In the implementation block
+(id)mediaPreviewInfoListWithStatuses:(id)arg1 options:(unsigned)arg2 ;
+(id)mediaPreviewInfoListWithStatus:(id)arg1 ;
+(id)mediaPreviewInfoListWithStatuses:(id)arg1 includeDummyStatuses:(char)arg2 options:(unsigned)arg3 ;
+(id)mediaPreviewInfoListWithStatuses:(id)arg1 ;
-(id)imageFullSizeURL;
-(CGSize)imageFullSize;
-(id)entityURL;
-(id)imageThumbnailURL;
-(unsigned)mediaIndex;
-(id)_mediaInfo;
-(void)setMediaIndex:(unsigned)arg1 ;
-(id)altText;
-(char)isEqual:(id)arg1 ;
-(TFNTwitterStatus *)status;
-(void)setStatus:(TFNTwitterStatus *)arg1 ;
@end

