/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 9:31:30 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/8A9DE0F0-8C0D-4043-A8B5-545C839396E1/Twitter.app/Twitter
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <TFNUI/TFNTableViewCell.h>

@protocol T1OnboardingSearchViewDelegate;
@class T1OnboardingSearchView;

@interface T1FollowsAdvancedUserSearchCell : TFNTableViewCell {

	id<T1OnboardingSearchViewDelegate> _delegate;
	T1OnboardingSearchView* _userSearchView;

}

@property (assign,nonatomic,__weak) id<T1OnboardingSearchViewDelegate> delegate;              //@synthesize delegate=_delegate - In the implementation block
@property (nonatomic,readonly) T1OnboardingSearchView * userSearchView;                       //@synthesize userSearchView=_userSearchView - In the implementation block
-(T1OnboardingSearchView *)userSearchView;
-(void)setDelegate:(id<T1OnboardingSearchViewDelegate>)arg1 ;
-(id)initWithStyle:(int)arg1 reuseIdentifier:(id)arg2 ;
-(id<T1OnboardingSearchViewDelegate>)delegate;
@end

