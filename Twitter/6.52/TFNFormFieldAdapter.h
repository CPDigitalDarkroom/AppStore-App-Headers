/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 14, 2016 at 9:31:46 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/8A9DE0F0-8C0D-4043-A8B5-545C839396E1/Twitter.app/Twitter
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Twitter/Twitter-Structs.h>
#import <TFNUI/TFNItemsCollectionItemAdapter.h>

@class NSIndexPath;

@interface TFNFormFieldAdapter : TFNItemsCollectionItemAdapter {

	float _previousCombiningWidth;
	NSIndexPath* _previousCombiningIndexPath;
	char _requiredImmediateInput;

}

@property (assign,nonatomic) char requiredImmediateInput;              //@synthesize requiredImmediateInput=_requiredImmediateInput - In the implementation block
-(id)_existingFormFieldCellInCollectionView:(id)arg1 forFormField:(id)arg2 ;
-(char)requiredImmediateInput;
-(void)setRequiredImmediateInput:(char)arg1 ;
-(id)_pickerViewControllerForFormField:(id)arg1 ;
-(CGSize)dataViewController:(id)arg1 collectionViewSizeForItem:(id)arg2 constrainedToSize:(CGSize)arg3 withOptions:(id)arg4 atIndexPath:(id)arg5 ;
-(id)dataViewController:(id)arg1 collectionViewCellForItem:(id)arg2 withOptions:(id)arg3 atIndexPath:(id)arg4 ;
-(void)dataViewController:(id)arg1 didSelectItem:(id)arg2 withOptions:(id)arg3 atIndexPath:(id)arg4 ;
-(void)dataViewController:(id)arg1 willDisplayCell:(id)arg2 forItem:(id)arg3 withOptions:(id)arg4 atIndexPath:(id)arg5 ;
@end

