/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:49:44 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/FFD4619E-59E7-49B9-AD90-0E28F5807C56/YouTube.app/YouTube
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <YouTube/PBGeneratedMessage.h>

@class YTIButtonSupportedRenderers, YTIThumbnailDetails, YTIFormattedString, YTINavigationEndpoint, NSData;

@interface YTICommentSimpleboxRenderer : PBGeneratedMessage

@property (assign,nonatomic) char hasSubmitButton; 
@property (nonatomic,retain) YTIButtonSupportedRenderers * submitButton; 
@property (assign,nonatomic) char hasCancelButton; 
@property (nonatomic,retain) YTIButtonSupportedRenderers * cancelButton; 
@property (assign,nonatomic) char hasAuthorThumbnail; 
@property (nonatomic,retain) YTIThumbnailDetails * authorThumbnail; 
@property (assign,nonatomic) char hasPlaceholderText; 
@property (nonatomic,retain) YTIFormattedString * placeholderText; 
@property (assign,nonatomic) char hasPrepareAccountEndpoint; 
@property (nonatomic,retain) YTINavigationEndpoint * prepareAccountEndpoint; 
@property (assign,nonatomic) char hasTrackingParams; 
@property (nonatomic,retain) NSData * trackingParams; 
+(id)descriptor;
@end

