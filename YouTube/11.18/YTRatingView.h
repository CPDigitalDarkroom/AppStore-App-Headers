/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:49:31 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/FFD4619E-59E7-49B9-AD90-0E28F5807C56/YouTube.app/YouTube
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <YouTube/YouTube-Structs.h>
#import <UIKit/UIView.h>

@class UIImageView, UIColor;

@interface YTRatingView : UIView {

	UIImageView* _imageView;
	char _invertColors;
	UIColor* _filledColor;
	UIColor* _emptyColor;
	double _rating;

}

@property (assign,nonatomic) double rating;                      //@synthesize rating=_rating - In the implementation block
@property (nonatomic,retain) UIColor * filledColor;              //@synthesize filledColor=_filledColor - In the implementation block
@property (nonatomic,retain) UIColor * emptyColor;               //@synthesize emptyColor=_emptyColor - In the implementation block
@property (assign,nonatomic) char invertColors;                  //@synthesize invertColors=_invertColors - In the implementation block
-(void)setInvertColors:(char)arg1 ;
-(char)invertColors;
-(id)init;
-(void)layoutSubviews;
-(CGSize)sizeThatFits:(CGSize)arg1 ;
-(double)rating;
-(void)setRating:(double)arg1 ;
-(void)setEmptyColor:(UIColor *)arg1 ;
-(void)setFilledColor:(UIColor *)arg1 ;
-(UIColor *)emptyColor;
-(UIColor *)filledColor;
@end

