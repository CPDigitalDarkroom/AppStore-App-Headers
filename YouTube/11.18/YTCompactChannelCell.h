/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:49:30 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/FFD4619E-59E7-49B9-AD90-0E28F5807C56/YouTube.app/YouTube
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <YouTube/YouTube-Structs.h>
#import <YouTube/YTCollectionViewCell.h>
#import <YouTube/YTThumbnailMapping.h>
#import <YouTube/YTSubscribableChannelCell.h>
#import <YouTube/YTFeedViewCellProtocol.h>

@protocol YTResponder;
@class YTICompactChannelRenderer, YTImageView, YTCompactChannelMetadataView, YTSubscribeSwitch, NSString;

@interface YTCompactChannelCell : YTCollectionViewCell <YTThumbnailMapping, YTSubscribableChannelCell, YTFeedViewCellProtocol> {

	YTICompactChannelRenderer* _entry;
	YTImageView* _thumbnailImageView;
	YTCompactChannelMetadataView* _channelMetadataView;
	YTSubscribeSwitch* _subscribeSwitch;
	id<YTResponder> _parentResponder;

}

@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
@property (nonatomic,__weak,readonly) id<YTResponder> parentResponder; 
+(CGSize)cellSizeForEntry:(id)arg1 size:(CGSize)arg2 ;
+(CGSize)cellSizeForEntry_regular:(id)arg1 size:(CGSize)arg2 ;
+(CGSize)cellSizeForEntry_compact:(id)arg1 size:(CGSize)arg2 ;
-(id<YTResponder>)parentResponder;
-(id)thumbnailMappings;
-(void)setLocator:(id)arg1 ;
-(void)enableSubscribeSwitch;
-(id)subscribeSwitch;
-(void)setParentResponder:(id<YTResponder>)arg1 ;
-(void)layoutSubviews_regular;
-(void)layoutSubviews_compact;
-(void)setEntry:(id)arg1 ;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)layoutSubviews;
-(void)prepareForReuse;
-(float)thumbnailSize;
-(id)entry;
@end

