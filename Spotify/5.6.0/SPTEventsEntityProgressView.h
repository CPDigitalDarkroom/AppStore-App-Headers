/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:33 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Spotify/Spotify-Structs.h>
#import <UIKit/UIView.h>

@class NSTimer, UIProgressView;

@interface SPTEventsEntityProgressView : UIView {

	char _hasFinishedLoading;
	NSTimer* _timer;
	UIProgressView* _progressView;

}

@property (assign,nonatomic) char hasFinishedLoading;                    //@synthesize hasFinishedLoading=_hasFinishedLoading - In the implementation block
@property (nonatomic,retain) NSTimer * timer;                            //@synthesize timer=_timer - In the implementation block
@property (nonatomic,retain) UIProgressView * progressView;              //@synthesize progressView=_progressView - In the implementation block
-(void)setHasFinishedLoading:(char)arg1 ;
-(void)incrementProgress;
-(float)newProgressValue;
-(char)hasFinishedLoading;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)dealloc;
-(void)layoutSubviews;
-(void)setProgressView:(UIProgressView *)arg1 ;
-(UIProgressView *)progressView;
-(void)setTimer:(NSTimer *)arg1 ;
-(NSTimer *)timer;
-(void)startLoading;
-(void)finishLoading;
@end

