/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:45 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Spotify/SettingsSection.h>

@class NSArray;

@interface MultipleChoiceSettingsSection : SettingsSection {

	int _selectedChoiceIndex;
	NSArray* _cells;

}

@property (assign,nonatomic) int selectedChoiceIndex;              //@synthesize selectedChoiceIndex=_selectedChoiceIndex - In the implementation block
@property (nonatomic,readonly) int numberOfChoices; 
@property (nonatomic,retain) NSArray * cells;                      //@synthesize cells=_cells - In the implementation block
-(id)initWithSettingsViewController:(id)arg1 ;
-(void)didSelectRow:(int)arg1 ;
-(void)reloadChoices;
-(int)numberOfChoices;
-(void)setSelectionState:(char)arg1 onCell:(id)arg2 ;
-(int)selectedChoiceIndex;
-(void)setSelectedChoiceIndex:(int)arg1 ;
-(id)titleForChoiceIndex:(int)arg1 ;
-(id)subtitleForChoiceIndex:(int)arg1 ;
-(void)setCells:(NSArray *)arg1 ;
-(NSArray *)cells;
-(id)cellForRow:(int)arg1 ;
-(int)numberOfRows;
@end

