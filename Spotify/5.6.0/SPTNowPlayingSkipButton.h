/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:39 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Spotify/Spotify-Structs.h>
#import <Spotify/SPTNowPlayingButton.h>

@class UIColor;

@interface SPTNowPlayingSkipButton : SPTNowPlayingButton {

	char _forward;
	int _icon;
	UIColor* _iconColor;
	CGSize _iconSize;

}

@property (getter=isForward,nonatomic,readonly) char forward;              //@synthesize forward=_forward - In the implementation block
+(id)skipForwardButton;
+(id)skipBackButton;
+(id)button;
-(void)setIconColor:(id)arg1 ;
-(id)iconColor;
-(id)initWithFrame:(CGRect)arg1 forward:(char)arg2 ;
-(id)accessibilityLabel;
-(void)setIcon:(int)arg1 ;
-(int)icon;
-(void)setIconSize:(CGSize)arg1 ;
-(char)isForward;
-(CGSize)iconSize;
@end

