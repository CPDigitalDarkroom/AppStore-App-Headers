/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:51 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Spotify/Spotify-Structs.h>
#import <UIKit/UICollectionView.h>
#import <Spotify/SPTThemableView.h>

@protocol SPTThemableViewLayoutDelegate;
@class NSString;

@interface SPTCollectionView : UICollectionView <SPTThemableView> {

	char _hasHeaderViews;
	char _hasFooterViews;
	id<SPTThemableViewLayoutDelegate> _layoutDelegate;

}

@property (assign,nonatomic) char hasHeaderViews;                                                  //@synthesize hasHeaderViews=_hasHeaderViews - In the implementation block
@property (assign,nonatomic) char hasFooterViews;                                                  //@synthesize hasFooterViews=_hasFooterViews - In the implementation block
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
@property (assign,nonatomic,__weak) id<SPTThemableViewLayoutDelegate> layoutDelegate;              //@synthesize layoutDelegate=_layoutDelegate - In the implementation block
-(void)applyThemeLayout;
-(id)initWithDefaultLayoutAndFrame:(CGRect)arg1 headerViews:(char)arg2 footerViews:(char)arg3 ;
-(char)hasHeaderViews;
-(void)setHasHeaderViews:(char)arg1 ;
-(char)hasFooterViews;
-(void)setHasFooterViews:(char)arg1 ;
-(id<SPTThemableViewLayoutDelegate>)layoutDelegate;
-(void)setLayoutDelegate:(id<SPTThemableViewLayoutDelegate>)arg1 ;
-(void)dealloc;
-(id)initWithFrame:(CGRect)arg1 collectionViewLayout:(id)arg2 ;
@end

