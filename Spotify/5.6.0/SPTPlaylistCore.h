/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:53 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


#import <Spotify/Spotify-Structs.h>
@class SPSession, SPCore;

@interface SPTPlaylistCore : NSObject {

	spotify_resolver* _playlist_resolver;
	shared_ptr<spotify::api::CResolverWrapper>* _playlist_resolver_cpp;
	int _playlist_resolver_token;
	SPSession* _session;
	SPCore* _core;

}

@property (assign,nonatomic,__weak) SPSession * session;              //@synthesize session=_session - In the implementation block
@property (assign,nonatomic,__weak) SPCore * core;                    //@synthesize core=_core - In the implementation block
-(SPCore *)core;
-(void)setCore:(SPCore *)arg1 ;
-(id)initWithSession:(id)arg1 core:(id)arg2 ;
-(void)registerPlaylistResolverWithRouter:(id)arg1 ;
-(void)unregisterPlaylistResolverWithRouter:(id)arg1 ;
-(SPSession *)session;
-(void)setSession:(SPSession *)arg1 ;
@end

