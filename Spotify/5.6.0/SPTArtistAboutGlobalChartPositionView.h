/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:29 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Spotify/Spotify-Structs.h>
#import <UIKit/UIView.h>
#import <Spotify/GLUEStyleable.h>

@class NSNumberFormatter, SPTLayoutConstraintBuilder, UIView, CAShapeLayer, GLUELabel, NSString;

@interface SPTArtistAboutGlobalChartPositionView : UIView <GLUEStyleable> {

	unsigned _globalChartPosition;
	NSNumberFormatter* _numberFormatter;
	SPTLayoutConstraintBuilder* _layout;
	UIView* _circleView;
	CAShapeLayer* _circleLayer;
	GLUELabel* _rankLabel;
	GLUELabel* _textLabel;

}

@property (assign,nonatomic) unsigned globalChartPosition;                     //@synthesize globalChartPosition=_globalChartPosition - In the implementation block
@property (nonatomic,retain) NSNumberFormatter * numberFormatter;              //@synthesize numberFormatter=_numberFormatter - In the implementation block
@property (nonatomic,retain) SPTLayoutConstraintBuilder * layout;              //@synthesize layout=_layout - In the implementation block
@property (nonatomic,retain) UIView * circleView;                              //@synthesize circleView=_circleView - In the implementation block
@property (nonatomic,retain) CAShapeLayer * circleLayer;                       //@synthesize circleLayer=_circleLayer - In the implementation block
@property (nonatomic,retain) GLUELabel * rankLabel;                            //@synthesize rankLabel=_rankLabel - In the implementation block
@property (nonatomic,retain) GLUELabel * textLabel;                            //@synthesize textLabel=_textLabel - In the implementation block
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(unsigned)globalChartPosition;
-(void)glue_applyStyle:(id)arg1 ;
-(void)addViewConstraints;
-(void)setGlobalChartPosition:(unsigned)arg1 ;
-(UIView *)circleView;
-(GLUELabel *)rankLabel;
-(CAShapeLayer *)circleLayer;
-(void)setCircleView:(UIView *)arg1 ;
-(void)setCircleLayer:(CAShapeLayer *)arg1 ;
-(void)setRankLabel:(GLUELabel *)arg1 ;
-(void)setNumberFormatter:(NSNumberFormatter *)arg1 ;
-(NSNumberFormatter *)numberFormatter;
-(id)initWithFrame:(CGRect)arg1 ;
-(GLUELabel *)textLabel;
-(SPTLayoutConstraintBuilder *)layout;
-(void)setLayout:(SPTLayoutConstraintBuilder *)arg1 ;
-(void)setTextLabel:(GLUELabel *)arg1 ;
@end

