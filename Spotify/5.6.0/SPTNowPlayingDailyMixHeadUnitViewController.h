/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:40 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Spotify/SPTNowPlayingBaseHeadUnitViewController.h>
#import <Spotify/SPTNowPlayingRemoteControlPolicyUpdateOserver.h>

@protocol SPTUpsellManager, SPTNowPlayingRemoteControlPolicy;
@class SPTNowPlayingModel, SPTNowPlayingBaseHeadUnitController, SPTLogCenter, SPTProgressView, SPTNowPlayingBaseHeadUnitView, NSString;

@interface SPTNowPlayingDailyMixHeadUnitViewController : SPTNowPlayingBaseHeadUnitViewController <SPTNowPlayingRemoteControlPolicyUpdateOserver> {

	SPTNowPlayingModel* _model;
	SPTNowPlayingBaseHeadUnitController* _headUnitController;
	id<SPTUpsellManager> _upsellManager;
	id<SPTNowPlayingRemoteControlPolicy> _remoteControlPolicy;
	SPTLogCenter* _logCenter;
	SPTProgressView* _progressView;

}

@property (nonatomic,retain) SPTNowPlayingModel * model;                                            //@synthesize model=_model - In the implementation block
@property (nonatomic,retain) SPTNowPlayingBaseHeadUnitView * view; 
@property (nonatomic,retain) SPTNowPlayingBaseHeadUnitController * headUnitController;              //@synthesize headUnitController=_headUnitController - In the implementation block
@property (assign,nonatomic,__weak) id<SPTUpsellManager> upsellManager;                             //@synthesize upsellManager=_upsellManager - In the implementation block
@property (nonatomic,retain) id<SPTNowPlayingRemoteControlPolicy> remoteControlPolicy;              //@synthesize remoteControlPolicy=_remoteControlPolicy - In the implementation block
@property (assign,nonatomic,__weak) SPTLogCenter * logCenter;                                       //@synthesize logCenter=_logCenter - In the implementation block
@property (nonatomic,retain) SPTProgressView * progressView;                                        //@synthesize progressView=_progressView - In the implementation block
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(SPTLogCenter *)logCenter;
-(void)setLogCenter:(SPTLogCenter *)arg1 ;
-(id<SPTUpsellManager>)upsellManager;
-(void)setUpsellManager:(id<SPTUpsellManager>)arg1 ;
-(void)setHeadUnitController:(SPTNowPlayingBaseHeadUnitController *)arg1 ;
-(SPTNowPlayingBaseHeadUnitController *)headUnitController;
-(void)needUpdateUI;
-(id)provideRemoteControlPolicy;
-(id<SPTNowPlayingRemoteControlPolicy>)remoteControlPolicy;
-(void)remoteControlPolicyDidChange:(id)arg1 ;
-(void)setRemoteControlPolicy:(id<SPTNowPlayingRemoteControlPolicy>)arg1 ;
-(id)initWithModel:(id)arg1 theme:(id)arg2 logCenter:(id)arg3 upsellManager:(id)arg4 remoteControlPolicy:(id)arg5 ;
-(void)radioThumbButtonTouchedUpInside:(unsigned)arg1 ;
-(void)negativeFeedbackButtonTouchedUpInside:(id)arg1 ;
-(void)positiveFeedbackButtonTouchedUpInside:(id)arg1 ;
-(void)showImprovingMixProgressView;
-(void)hideImprovingMixProgressViewWithError:(id)arg1 ;
-(void)dealloc;
-(void)loadView;
-(SPTNowPlayingModel *)model;
-(void)viewWillAppear:(char)arg1 ;
-(void)viewDidLoad;
-(void)setProgressView:(SPTProgressView *)arg1 ;
-(SPTProgressView *)progressView;
-(void)setModel:(SPTNowPlayingModel *)arg1 ;
@end

