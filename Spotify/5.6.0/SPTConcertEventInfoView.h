/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:33 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Spotify/Spotify-Structs.h>
#import <UIKit/UIView.h>

@class NSDate, SPTTheme, UILabel, SPTCalendarView, NSString;

@interface SPTConcertEventInfoView : UIView {

	NSDate* _date;
	SPTTheme* _theme;
	UILabel* _titleLabel;
	UILabel* _subtitleLabel;
	SPTCalendarView* _calendarView;

}

@property (nonatomic,copy) NSString * title; 
@property (nonatomic,copy) NSString * subtitle; 
@property (nonatomic,retain) NSDate * date;                               //@synthesize date=_date - In the implementation block
@property (nonatomic,retain) SPTTheme * theme;                            //@synthesize theme=_theme - In the implementation block
@property (nonatomic,retain) UILabel * titleLabel;                        //@synthesize titleLabel=_titleLabel - In the implementation block
@property (nonatomic,retain) UILabel * subtitleLabel;                     //@synthesize subtitleLabel=_subtitleLabel - In the implementation block
@property (nonatomic,retain) SPTCalendarView * calendarView;              //@synthesize calendarView=_calendarView - In the implementation block
-(void)setupTitleLabel;
-(void)setupSubtitleLabel;
-(void)setupCalendarViewWithDate:(id)arg1 ;
-(void)setCalendarView:(SPTCalendarView *)arg1 ;
-(SPTCalendarView *)calendarView;
-(id)initWithFrame:(CGRect)arg1 ;
-(id)init;
-(void)setTitle:(NSString *)arg1 ;
-(NSString *)title;
-(NSDate *)date;
-(UILabel *)titleLabel;
-(NSString *)subtitle;
-(UILabel *)subtitleLabel;
-(void)setDate:(NSDate *)arg1 ;
-(void)setSubtitle:(NSString *)arg1 ;
-(void)setTitleLabel:(UILabel *)arg1 ;
-(void)setSubtitleLabel:(UILabel *)arg1 ;
-(SPTTheme *)theme;
-(void)setTheme:(SPTTheme *)arg1 ;
@end

