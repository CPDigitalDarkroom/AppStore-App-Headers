/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:43 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Spotify/SPAction.h>

@protocol SPTPlayer, SPTPlayerQueue, SPTUpsellManager;
@class SPTPlayerTrack;

@interface SPTQueueTrackAction : SPAction {

	char _trackIsAvailable;
	SPTPlayerTrack* _track;
	id<SPTPlayer> _player;
	id<SPTPlayerQueue> _playerQueue;
	id<SPTUpsellManager> _upsellManager;

}

@property (nonatomic,retain) SPTPlayerTrack * track;                          //@synthesize track=_track - In the implementation block
@property (nonatomic,retain) id<SPTPlayer> player;                            //@synthesize player=_player - In the implementation block
@property (nonatomic,retain) id<SPTPlayerQueue> playerQueue;                  //@synthesize playerQueue=_playerQueue - In the implementation block
@property (nonatomic,retain) id<SPTUpsellManager> upsellManager;              //@synthesize upsellManager=_upsellManager - In the implementation block
@property (assign,nonatomic) char trackIsAvailable;                           //@synthesize trackIsAvailable=_trackIsAvailable - In the implementation block
-(id<SPTUpsellManager>)upsellManager;
-(id<SPTPlayerQueue>)playerQueue;
-(id)logEventName;
-(char)executeBeforeDismiss;
-(void)setUpsellManager:(id<SPTUpsellManager>)arg1 ;
-(void)setPlayerQueue:(id<SPTPlayerQueue>)arg1 ;
-(id)initWithTrack:(id)arg1 player:(id)arg2 playerQueue:(id)arg3 upsellManager:(id)arg4 logContext:(id)arg5 ;
-(char)trackIsAvailable;
-(void)setTrackIsAvailable:(char)arg1 ;
-(SPTPlayerTrack *)track;
-(void)setPlayer:(id<SPTPlayer>)arg1 ;
-(id<SPTPlayer>)player;
-(id)title;
-(id)image;
-(id)execute:(id)arg1 ;
-(void)setTrack:(SPTPlayerTrack *)arg1 ;
@end

