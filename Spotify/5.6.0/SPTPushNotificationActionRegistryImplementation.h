/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:19 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Spotify/SPTPushNotificationActionRegistry.h>

@class NSMapTable, NSMutableArray, NSString;

@interface SPTPushNotificationActionRegistryImplementation : NSObject <SPTPushNotificationActionRegistry> {

	char _enabled;
	NSMapTable* _actions;
	NSMapTable* _allocationContexts;
	NSMutableArray* _notifications;

}

@property (assign,getter=isEnabled,nonatomic) char enabled;                //@synthesize enabled=_enabled - In the implementation block
@property (nonatomic,retain) NSMapTable * actions;                         //@synthesize actions=_actions - In the implementation block
@property (nonatomic,retain) NSMapTable * allocationContexts;              //@synthesize allocationContexts=_allocationContexts - In the implementation block
@property (nonatomic,retain) NSMutableArray * notifications;               //@synthesize notifications=_notifications - In the implementation block
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
+(id)sharedRegistry;
-(NSMapTable *)allocationContexts;
-(void)sendActionForIdentifier:(id)arg1 URL:(id)arg2 completionBlock:(/*^block*/id)arg3 ;
-(void)clearPendingActions;
-(void)registerAction:(id)arg1 forIdentifier:(id)arg2 ;
-(void)setAllocationContexts:(NSMapTable *)arg1 ;
-(void)setActions:(NSMapTable *)arg1 ;
-(id)init;
-(NSMapTable *)actions;
-(void)setEnabled:(char)arg1 ;
-(char)isEnabled;
-(NSMutableArray *)notifications;
-(void)setNotifications:(NSMutableArray *)arg1 ;
@end

