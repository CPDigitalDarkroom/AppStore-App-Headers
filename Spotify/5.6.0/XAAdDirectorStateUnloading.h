/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:21 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Spotify/XAAdDirectorState.h>

@class NSError;

@interface XAAdDirectorStateUnloading : XAAdDirectorState {

	char recognizerDestroyed;
	char tornDown;
	NSError* adCompletionError;
	int _stateType;

}

@property (assign,nonatomic) char recognizerDestroyed; 
@property (assign,nonatomic) char tornDown; 
@property (nonatomic,retain) NSError * adCompletionError; 
-(void)runState;
-(void)tearDownState;
-(void)handlePlayAdvertisement:(id)arg1 ;
-(void)handlePauseAdvertisement;
-(void)handleResumeAdvertisement;
-(void)handleCancelAdvertisement;
-(char)adPresented;
-(char)adPlaying;
-(void)setAdCompletionError:(NSError *)arg1 ;
-(void)setRecognizerDestroyed:(char)arg1 ;
-(void)setTornDown:(char)arg1 ;
-(void)movetoResetState;
-(NSError *)adCompletionError;
-(char)recognizerDestroyed;
-(char)tornDown;
-(id)init;
-(void)handleEvent:(id)arg1 ;
-(int)stateType;
@end

