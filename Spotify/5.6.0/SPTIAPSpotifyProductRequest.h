/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, May 18, 2016 at 11:45:36 PM Eastern Daylight Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /var/mobile/Containers/Bundle/Application/D2238F36-ED84-40F4-9CB2-8C71CACC8B47/Spotify.app/Spotify
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@protocol SPTIAPSpotifyProductRequestDelegate;
@interface SPTIAPSpotifyProductRequest : NSObject {

	id<SPTIAPSpotifyProductRequestDelegate> _delegate;

}

@property (assign,nonatomic,__weak) id<SPTIAPSpotifyProductRequestDelegate> delegate;              //@synthesize delegate=_delegate - In the implementation block
+(id)productRequestWithDataLoaderFactory:(id)arg1 ;
+(id)productRequestWithHermesController:(id)arg1 ;
-(id)productIdentifiersFromResponseData:(id)arg1 error:(id*)arg2 ;
-(void)setDelegate:(id<SPTIAPSpotifyProductRequestDelegate>)arg1 ;
-(id<SPTIAPSpotifyProductRequestDelegate>)delegate;
-(void)start;
@end

